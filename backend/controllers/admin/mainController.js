...
class MainController {
  ...
  static orderlist = async (req, res) => {
    try {
      let records = await Order.find({}).sort({
        created_at: -1,
      });
      const admin = await Adminauth.find({});
      const settingRecord = await Setting.findOne({});
  ...
  static view = async (req, res) => {
    try {
      ...
      let record = await Order.findById(userId);
      let prdInfo  = record.cart ? JSON.parse(record.cart) :''
      const admin = await Adminauth.find({});
      const settingRecord = await Setting.findOne({});
      ...
      return res.render("admin/order_details", {
        record,
        admin,
        userId,
        orders,
